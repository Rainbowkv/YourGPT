...
class BigramLanguageModel(nn.Module):
    ...
    def forward(self, idx, target=None):
        ...
        pos_emd = self.pos_embedding_table(torch.arange(block_size, device=device))  # T = block_size
        integral_emd = tok_emd + pos_emd  # (T, C) + (B, T, C)
        logits = self.lm_head(integral_emd)  # logits.shape = (B, T, vocab_size)
...
model = BigramLanguageModel().to(device)  # 模型实例
